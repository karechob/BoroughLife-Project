window.addEventListener('load', () => {
  ...
  if (boroughs.value == "Staten Island") {
    ...
    function AddElement (list) {
      ...
      thumbs_up.appendChild(myIcon);
      upvote_btn.appendChild(thumbs_up);
      upvote_btn.textContent = 'Like';
      upvote_btn.classList.add('btn-like', 'btn', 'border-2','ms-5', 'text-wrap', 'btn-outline-light');
      var clicked = false;
      var currentLikes = 0
      ...
      upvote_btn.addEventListener("click", function() {
        ...
        this.innerText = currentLikes + " Likes";
        if(currentLikes <= 1){
          this.innerText = currentLikes + " Like";
        }
        if(currentLikes == 0){
          this.innerText = "Like";
        }
      ...
      $(sub_con).fadeOut(3000, "linear");
      submittion_input.setAttribute("clear", "");
      console.log(sub_con);
      return;
    }
  })
