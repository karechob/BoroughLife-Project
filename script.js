window.addEventListener('load', () => {
  ...
  myIcon.innerHTML = `<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-hand-thumbs-up" viewBox="0 0 16 16">
...
function AddElement (list) {
  ...
  const thumbs_up = document.createElement("span");

  upvote_btn.classList.add('btn-like', 'btn', 'ms-5', 'btn-outline-light');
  

  upvote_btn.appendChild(thumbs_up);
  upvote_btn.textContent = 'up-vote';
  var count = 0;

  upvote_btn.addEventListener('click', () => {
    count++;
    console.log(experience_text.value + ": " + count);
  })
  ...
  const like_counter = document.createElement("p");
  like_counter.classList.add('text-white', 'text-center', 'h5');
  like_counter.value = count.toString();
  submittion_el.appendChild(like_counter);

  const submittion_input = document.createElement("p");
    submittion_input.classList.add('text-white', 'text-center', 'h5');
